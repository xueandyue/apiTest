...
def add_case_data(type, **kwargs):
    ...
    try:
        if type:
            ...
            if case_opt.get_case_by_name(name, module, project) < 1:
                case_opt.insert_case(belong_module, **kwargs)
                logger.info('{name}用例添加成功: {kwargs}'.format(name=name, kwargs=kwargs))
            else:
                return '用例或配置已存在，请重新编辑'
            ...
            index = case_info.get('test_index')
            if name != case_opt.get_case_by_id(index, type=False) \
                    and case_opt.get_case_by_name(name, module, project) > 0:
                return '用例或配置已在该模块中存在，请重新命名'
            case_opt.update_case(belong_module, **kwargs)
            logger.info('{name}用例更新成功: {kwargs}'.format(name=name, kwargs=kwargs))
...
def add_test_reports(runner, report_name=None,username=None):
    """
    定时任务或者异步执行报告信息落地
    ...
    runner._summary['time']['start_datetime'] = datetime.datetime.fromtimestamp(time_stamp).strftime('%Y-%m-%d %H:%M:%S')
    ...
    report_path = os.path.join(os.getcwd(), "reports{}{}.html".format(separator, int(runner._summary['time']['start_at'])))
    # gen_html_report(runner._summary, report_template=os.path.join(os.getcwd(), r"templates{}Index/extent_report_template.html".format(separator)))
    render_html_report(runner._summary,report_template=os.path.join(os.getcwd(), r"templates{}Index/report_template.html".format(separator)))
    ...
    test_reports = {
        ...
        'testsRun': runner._summary.get('stat').get('teststeps').get('total'),
        'successes': runner._summary.get('stat').get('teststeps').get('successes'),
